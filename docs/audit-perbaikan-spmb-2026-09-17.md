...
| Log Aktivitas umum untuk pemindahan tahap dari halaman Peserta | SELESAI / LIVE | Jalur `admin.peserta.update-tahap` dan `admin.peserta.bulk-update-tahap` sekarang membuat log umum `tahapan.pindahkan_manual` per peserta dan satu ringkasan `tahapan.pindahkan_massal` untuk aksi bulk, tanpa menghapus riwayat teknis. Live di SHA `144afc1`. | Test regresi tersedia tetapi tidak dapat menjalankan assertion tanpa `SPMB_TEST_DB_PASSWORD`; lint, Pint, dan `git diff --check` lulus. |
| Peran `tim_spmb` | SELESAI / LIVE | Migration produksi `[40]` `2026_09_17_120000_add_tim_spmb_role_to_pengguna_table` sudah ran. Migration forward-only tersebut menambah `tim_spmb` pada enum MySQL/MariaDB tanpa mengubah akun yang ada; rollback menolak bila akun `tim_spmb` sudah tersedia. | Test regresi tersedia; eksekusinya dipagari secret test DB. |
